...
// this software.
// 
// Module: winx/ext/CreditStatic.cpp
// Creator: xushiwei
// Email: xushiweizh@gmail.com
// Date: 2006-8-19 20:26:15
// 
// $Id: CreditStatic.cpp,v 1.2 2006/08/20 04:59:06 xushiwei Exp $
// -----------------------------------------------------------------------*/
...
inline CCreditStatic::CCreditStatic()
{
...
inline BOOL CCreditStatic::StartScrolling()
{
	//if(m_ArrCredit.IsEmpty()) //@@code modify -->
	if (m_ArrCredit.empty())
		return FALSE;
	...
	//m_ArrIndex = m_ArrCredit.GetHeadPosition(); //@@code modify -->
	m_ArrIndex = 0;
	m_nCounter = 1;
	m_nClip = 0;
...
inline void CCreditStatic::EndScrolling()
{
...
inline void CCreditStatic::SetCredits(LPCTSTR credits,char delimiter)
{
	char *str,*ptr1,*ptr2;
	...
	//m_ArrCredit.RemoveAll(); //@@code modify -->
	m_ArrCredit.clear();

	ptr1 = str;
	while((ptr2 = strchr(ptr1,delimiter)) != NULL) {
		*ptr2 = '\0';
		//m_ArrCredit.AddTail(ptr1); //@@code modify -->
		m_ArrCredit.push_back(ptr1);
		ptr1 = ptr2+1;
	}
	//m_ArrCredit.AddTail(ptr1); //@@code modify -->
	m_ArrCredit.push_back(ptr1);
...
inline void CCreditStatic::SetCredits(UINT nID,char delimiter)
{
	CString credits;
...
inline void CCreditStatic::SetSpeed(UINT index, int speed)
{
...
inline void CCreditStatic::SetColor(UINT index, COLORREF col)
{
...
inline void CCreditStatic::SetTextHeight(UINT index, int height)
{
...
inline void CCreditStatic::SetEscape(UINT index, char escape)
{
...
inline void CCreditStatic::SetGradient(UINT value)
{
...
inline void CCreditStatic::SetTransparent(BOOL bTransparent)
{
	m_bTransparent = bTransparent;
}

//void CCreditStatic::OnPaint() //@@code modify -->
inline void winx_msg CCreditStatic::OnPaint(HWND hWnd)
{
...
//BOOL CCreditStatic::OnEraseBkgnd(CDC* pDC) //@@code modify -->
inline BOOL CCreditStatic::OnEraseBkgnd(HWND hWnd, HDC hDC) 
{
	return TRUE;
...
//************************************************************************
//void CCreditStatic::OnTimer(UINT nIDEvent) //@@code modify -->
inline void CCreditStatic::OnTimer(HWND hWnd, UINT nIDEvent) 
{
	if (nIDEvent != DISPLAY_TIMER_ID)
	{
		//CStatic::OnTimer(nIDEvent); //@@code modify -->
		Default(hWnd);
		return;
	}
	...
	if (!m_bProcessingBitmap) {
		...
		{
			m_nCounter=1;
			//m_szWork = m_ArrCredit.GetNext(m_ArrIndex); //@@code modify -->
			m_szWork = m_ArrCredit.at(m_ArrIndex++);
			if(m_bFirstTurn)
				bCheck = TRUE;
			//if(m_ArrIndex == NULL) { //@@code modify -->
			if (m_ArrIndex == m_ArrCredit.size()) {
				m_bFirstTurn = FALSE;
				//m_ArrIndex = m_ArrCredit.GetHeadPosition(); //@@code modify -->
				m_ArrIndex = 0;
			}
	...
	*/
	//@@code modify -->
	MoveCredit((CDC*)&dc, m_ScrollRect, m_ClientRect, bCheck);
	AddBackGround((CDC*)&dc, m_ScrollRect, m_ClientRect);
	
	//CStatic::OnTimer(nIDEvent); //@@code modify -->
	Default(hWnd);
}

inline void CCreditStatic::AddBackGround(CDC* pDC, CRect& m_ScrollRect, CRect& m_ClientRect)
{
	CDC memDC;
	...
	{
        //CBitmap* pOldBitmap = memDC.SelectObject( &m_BmpMain ); //@@code modify -->
		BitmapHandle pOldBitmap = memDC.SelectObject( &m_BmpMain );
   ...
   // Draw bitmap in the background if one has been set
	...
	//CBitmap* pOldMemDCBitmap = memDC.SelectObject( &bitmap ); //@@code modify -->
	BitmapHandle pOldMemDCBitmap = memDC.SelectObject( &bitmap );
		
	CDC tempDC;
	tempDC.CreateCompatibleDC(pDC);
	//CBitmap* pOldTempDCBitmap = tempDC.SelectObject( &m_BmpMain ); //@@code modify -->
	BitmapHandle pOldTempDCBitmap = tempDC.SelectObject( &m_BmpMain );
	...
	//CBitmap* pOldMaskDCBitmap = maskDC.SelectObject( &maskBitmap ); //@@code modify -->
	BitmapHandle pOldMaskDCBitmap = maskDC.SelectObject( &maskBitmap );
	...
	//CBitmap* pOldImageDCBitmap = imageDC.SelectObject( &bmpImage ); //@@code modify -->
	BitmapHandle pOldImageDCBitmap = imageDC.SelectObject( &bmpImage );
...
inline void CCreditStatic::DrawBitmap(CDC* pDC, CDC* pDC2, CRect *rBitmap)
{
...
inline void CCreditStatic::FillGradient(CDC *pDC, CRect *m_ScrollRect, CRect *m_FillRect, COLORREF color)
{ 
...
inline void CCreditStatic::MoveCredit(CDC* pDC, CRect& m_ScrollRect, CRect& m_ClientRect, BOOL bCheck)
{
	CDC memDC,memDC2;
    memDC.CreateCompatibleDC(pDC);
	...
	//CBitmap *pOldMemDCBitmap = NULL;
	//CBitmap *pOldMemDC2Bitmap = NULL; //@@code modify -->
	BitmapHandle pOldMemDCBitmap;
	BitmapHandle pOldMemDC2Bitmap;
	...
	if(m_BmpMain.m_hObject == NULL) {
		...
		//pOldMemDCBitmap = (CBitmap*)memDC.SelectObject(&m_BmpMain); //@@code modify -->
		pOldMemDCBitmap = memDC.SelectObject(&m_BmpMain);
	...
	//	FONT SELECTION
    CFont m_fntArial;
	//CFont* pOldFont = NULL; //@@code modify -->
	FontHandle pOldFont;
	BOOL bSuccess = FALSE;
...
inline void CCreditStatic::DrawCredit(CDC* pDC, CRect& m_ScrollRect)
{
	...
	//POSITION pos = m_ArrCredit.GetHeadPosition();  //@@code modify -->
	POSITION pos = 0;
	int height = 0;
	//while(pos != NULL && height <= m_ScrollRect.Height()) { //@@code modify -->
	while(pos != m_ArrCredit.size() && height <= m_ScrollRect.Height()) {
		//CString m_szWork = m_ArrCredit.GetNext(pos); //@@code modify -->
		CString m_szWork = m_ArrCredit.at(pos++);
		CFont	m_fntArial;
		//CFont	*pOldFont = NULL;
		//CBitmap *pOldMemDC2Bitmap = NULL; //@@code modify -->
		FontHandle	pOldFont;
...
//void CCreditStatic::OnDestroy() //@@code modify -->
inline void CCreditStatic::OnDestroy(HWND hWnd) 
{
	//CStatic::OnDestroy(); //@@code modify -->
	Default(hWnd);
...
inline BOOL CCreditStatic::SetBkImage(UINT nIDResource)
{
    return SetBkImage( (LPCTSTR)nIDResource );
}

inline BOOL CCreditStatic::SetBkImage(LPCTSTR lpszResourceName)
{
    ...
    //HBITMAP hBmp = (HBITMAP)::LoadImage( AfxGetInstanceHandle(), //@@code modify -->
	HBITMAP hBmp = (HBITMAP)::LoadImage( GetThisModule(), 
    ...
    else
    {
        ...
        //CBitmap* pOldMemDCBitmap = memDC.SelectObject( &m_bitmap ); //@@code modify -->
		BitmapHandle pOldMemDCBitmap = memDC.SelectObject( &m_bitmap );
...
// --------------------------------------------------------------------------
// $Log: CreditStatic.cpp,v $
// Revision 1.2  2006/08/20 04:59:06  xushiwei
// bugfix
//
// Revision 1.1  2006/08/20 04:49:57  xushiwei
// MFC-Compatibility:
